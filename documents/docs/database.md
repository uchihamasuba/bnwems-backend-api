...
model Equipment {
  ...
  replacementValue  Decimal             @default(0) @map("replacement_value") @db.Decimal(12, 2)
  status            String              @default("active") // active, inactive
  createdAt         DateTime            @default(now()) @map("created_at")
  updatedAt         DateTime            @updatedAt @map("updated_at")
...
model Order {
  ...
  status        String    @default("draft") // draft, confirmed, deposit_paid, in_progress, settlement_pending, completed, cancelled
...
model Settlement {
  ...
  changeAdjustment Decimal  @default(0) @map("change_adjustment") @db.Decimal(12, 2)
  additionalFee    Decimal  @default(0) @map("additional_fee") @db.Decimal(12, 2)
...
model Assignment {
  ...
  fieldStatus  String   @default("pending") @map("field_status") // pending, ready, in_setup, completed
  assignedAt   DateTime @default(now()) @map("assigned_at")

  workTask WorkTask     @relation(fields: [workTaskId], references: [workTaskId])
...
model WageSummary {
  ...
  totalWage      Decimal  @default(0) @map("total_wage") @db.Decimal(12, 2)
...
model SurveyReport {
  ...
  siteCondition   String?   @map("site_condition") @db.Text
  ...
  groundType      String?   @map("ground_type") @db.VarChar(100)
...
model ChangeRequest {
  ...
  noteFromLeader  String?   @map("note_from_leader") @db.Text
...
model ChangeRequestItem {
  ...
  equipmentItem Equipment @relation(fields: [equipmentItemId], references: [equipmentItemId])
...
model DamageLossReport {
  ...
  recordedBy        BigInt   @map("recorded_by")
  confirmedBy       BigInt?  @map("confirmed_by")
  totalCompensation Decimal  @default(0) @map("total_compensation") @db.Decimal(12, 2)
  status            String   @default("submitted") // submitted, confirmed
  createdAt         DateTime @default(now()) @map("created_at")
  updatedAt         DateTime @updatedAt @map("updated_at")
...
model DamageLossItem {
  ...
  compensationAmount        Decimal @default(0) @map("compensation_amount") @db.Decimal(12, 2)
  responsibleParty          String? @map("responsible_party") @db.VarChar(20) // customer, staff
...
model Notification {
  ...
  targetScreen   String?   @map("target_screen") @db.VarChar(50)
  targetRefType  String?   @map("target_ref_type") @db.VarChar(50)
  ...
  pushError      String?   @map("push_error") @db.VarChar(255)
  createdAt      DateTime  @default(now()) @map("created_at")
...
model Evidence {
  ...
  orderId         BigInt?  @map("order_id")
  storageProvider String   @default("firebase") @map("storage_provider") @db.VarChar(30)
