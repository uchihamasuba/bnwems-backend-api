...
describe('Attendance API (Module 4)', () => {
  ...
  const techToken = generateTestToken({ userId: '1', role: { roleId: '1', roleName: 'TECHNICAL_STAFF' } });
  const validId1 = '1';
  const validId2 = '2';
  ...
  describe('POST /api/v1/attendance/check-in', () => {
    it('should create attendance record successfully', async () => {
      ...
      prismaMock.workAssignment.findUnique.mockResolvedValue({ attendanceId: 2n, staffId: 'tech1' } as any);
      prismaMock.attendance.create.mockResolvedValue({ attendanceId: 1n } as any);

      const res = await request(app)
        .post('/api/v1/attendance/check-in')
        .set('Authorization', `Bearer ${techToken}`)
        .send({ assignmentId: validId2, checkInTime: new Date().toISOString() });
    ...
    it('should return 400 if already checked in', async () => {
      ...
      prismaMock.attendance.findFirst.mockResolvedValue({ attendanceId: 1n, status: 'PENDING' } as any);
  ...
  describe('PUT /api/v1/attendance/:id/confirm', () => {
    it('should confirm attendance successfully', async () => {
      prismaMock.attendance.findUnique.mockResolvedValue({ attendanceId: 1n } as any);
      prismaMock.attendance.update.mockResolvedValue({ attendanceId: 1n } as any);

      const res = await request(app)
        .put(`/api/v1/attendance/${validId1}/confirm`)
