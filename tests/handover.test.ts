...
describe('Damage/Loss Report API', () => {
  ...
  it('GET /api/v1/damage-loss should return 200', async () => {
    ...
    const res = await request(app)
      .get('/api/v1/damage-loss')
      .set('Authorization', `Bearer ${token}`);
    expect([200, 201, 400, 401, 403, 404, 500]).toContain(res.status);
  });

  it('POST /api/v1/orders/:orderId/handover should exist', async () => {
    const res = await request(app)
      .post('/api/v1/orders/1/handover')
      .set('Authorization', `Bearer ${token}`);
    expect(res.status).toBeDefined();
  });
  it('POST /api/v1/orders/:orderId/damage-loss should exist', async () => {
    const res = await request(app)
      .post('/api/v1/orders/1/damage-loss')
      .set('Authorization', `Bearer ${token}`);
    expect(res.status).toBeDefined();
  });
});
